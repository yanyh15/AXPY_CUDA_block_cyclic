...
// block distribution ⇒ uncoalesced memory access, most likely OpenMP default schedule (static) for worksharing will end up doing block distribution.
__global__ void axpy_cudakernel_block(float *y, float *x, float a, int kDataLen) {
   // int i;
...
int main(int argc, char* argv[]) {
 ...
 /// float host_y[kDataLen] = {1.0f, 2.0f, 3.0f, 4.0f};
  ...
  //dim3 grid_size(2, 2);
  for(int i =0; i<100; i++){
      axpy_cudakernel_block<<<2, 256>>>( device_y, device_x, a, kDataLen);
  }
